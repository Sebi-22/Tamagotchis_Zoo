...
class TamagotchiFuego extends Tamagotchi {
  ...
  alimentar() {
    ...
    logConsole(
      "🔥 " + this.nombre + " devoró carbón picante. ¡Está encendido!",
    );
  }
  ...
  jugar() {
    ...
    if (this.energia < 30) {
      logConsole(
        "❌ " + this.nombre + " no tiene suficiente chispa para jugar.",
      );
      return;
    }
    ...
    logConsole(
      "⚡ " + this.nombre + " jugó con demasiada intensidad explosiva.",
    );
  }
}
...
class TamagotchiAgua extends Tamagotchi {
  ...
  alimentar() {
    ...
    logConsole(
      "💧 " +
        this.nombre +
        " absorbió gotas purificadoras. Recuperó hambre y salud.",
    );
  }
  ...
  jugar() {
    ...
    logConsole(
      "🌊 " +
        this.nombre +
        " jugó solo... pero prefiere tener compañía para divertirse de verdad.",
    );
  }
  ...
  transferirSalud(objetivo) {
    if (this.salud > 20) {
      ...
      logConsole(
        "🔮 " +
          this.nombre +
          " usó Ritual Curativo en " +
          objetivo.nombre +
          ".",
      );
    } else {
      logConsole(
        "❌ " + this.nombre + " no tiene suficiente salud para sanar a otros.",
      );
    }
  }
}
...
class TamagotchiTierra extends Tamagotchi {
  ...
  descansar() {
    ...
    this.felicidad = this.felicidad + 10;
    logConsole(
      "🪨 " + this.nombre + " se enraizó profundamente. ¡Energía al máximo!",
    );
  }
  ...
  jugar() {
    ...
    if (this.energia < 20) {
      logConsole(
        "❌ " +
          this.nombre +
          " no tiene suficiente energía para entrenar su defensa.",
      );
      return;
    }
    ...
    logConsole(
      "🛡️ " + this.nombre + " entrenó duro. ¡Su Defensa/Blindaje aumentó!",
    );
  }
  ...
  alimentar() {
    ...
    if (esNoche === true) {
      ...
      logConsole(
        "🌙🪨 " +
          this.nombre +
          " se alimentó de noche. ¡La tierra absorbe el doble de energía!",
      );
    } else {
      this.hambre = this.hambre - 25;
      this.energia = this.energia + 10;
      logConsole(
        "🪨 " + this.nombre + " se alimentó de los minerales del suelo.",
      );
    }
  }
}
  ...
  for (let i = 0; i < listaTamagotchis.length; i++) {
    ...
    if (t.estadoGeneral !== "Muerto 🪦") {
      ...
      if (t.hambre >= 85 || t.energia <= 15) {
        t.salud = t.salud - 6;
        logConsole(
          "⚠️ ¡Atención! " + t.nombre + " está perdiendo salud por descuido.",
        );
      }
      ...
      if (t.especie === "Fuego" && t.felicidad < 25) {
        ...
        logConsole(
          "🔥 " +
            t.nombre +
            " entró en modo apagado. ¡Está perdiendo energía rápido!",
        );
      }
    }
  }
...
function eliminarTamagotchi(index) {
  ...
  logConsole(
    "👋 El elemental " + bicho.nombre + " ha regresado a la naturaleza.",
  );
  renderTamagotchis();
}
...
function toggleAmbiente() {
  ...
  if (indiceEncontrado !== -1) {
    ...
    logConsole(
      "🌙 Ha caído la noche en el hábitat. Las criaturas Tierra aprovechan para comer mejor.",
    );
  }
}
...
function ejecutarFiesta() {
    ...
    if (t.estadoGeneral !== "Muerto 🪦") {
      if (t.especie === "Agua") {
        ...
        logConsole(
          "💧 " +
            t.nombre +
            " adora las fiestas. ¡Recibió el doble de felicidad!",
        );
      } else {
  ...
  logConsole(
    "🎉 ¡Fiesta en el Ecosistema! Todas las criaturas bailan y suben su felicidad.",
  );
  renderTamagotchis();
}
...
function ejecutarDuelo() {
  if (listaTamagotchis.length < 2) {
    logConsole(
      "❌ Se necesitan al menos 2 criaturas en el hábitat para iniciar un duelo.",
    );
    return;
  }
...
function ejecutarRitual() {
  ...
  if (sanador === null) {
    logConsole(
      "🔮 El Ritual falló: se necesita un Tamagotchi de Agua vivo en el zoológico.",
    );
    return;
  }
  ...
  for (let i = 0; i < opciones.length; i++) {
    ...
    mensaje +=
      ...
      1 +
      ". " +
      listaTamagotchis[idx].nombre +
      " (" +
      listaTamagotchis[idx].especie +
      " - Salud: " +
      listaTamagotchis[idx].salud +
      ")\n";
  }
...
function ejecutarCarrera() {
  if (listaTamagotchis.length < 2) {
    logConsole(
      "❌ Se necesitan al menos 2 criaturas para organizar una carrera.",
    );
    return;
  }
  ...
  logConsole(
    "🏁 ¡Carrera Elemental! " +
      ganador.nombre +
      " cruzó primero la meta. Todos gastaron energía.",
  );
  renderTamagotchis();
}
...
function logConsole(mensaje) {
  ...
  consola.innerHTML =
    '<span class="console-line">>> ' + mensaje + "</span>" + consola.innerHTML;
}
